...
class SceneManager(BaseStructure):
    def __init__(self):
        ...
        self.menu_references = {i.__name__.lower(): i for i in [getattr(this, j) for j in dir(this)] if
                                isinstance(i, MetaClass) and not isinstance(i, Scene)}
        ...
        self.menu.enter()
        self.default_bg = pygame.Surface([Config.WIDTH, Config.HEIGHT])
    ...
    def update(self, events: list[pygame.event.Event], dt):
        # self.camera_shake_intensity *= 0.8 ** dt
        self.camera_shake_intensity = lerp(self.camera_shake_intensity, 0, 0.1 * dt)
        ...
        for e in events:
            ...
            if e.type == CAMERA_SHAKE:
                self.camera_shake_intensity = e.intensity
